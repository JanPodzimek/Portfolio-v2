...
const CertificateCard: React.FC<CertificateCardProps> = ({ title, issuer, date, url, iconSrc, defaultIcon }) => {
  return (
    <a 
      ...
      className="block p-6 bg-white dark:bg-gray-800 rounded-lg shadow-lg hover:shadow-2xl hover:scale-105 transform transition-all duration-300 ease-in-out group h-full flex flex-col"
    >
      <div className="flex items-start space-x-4 flex-grow">
        ...
        <FaExternalLinkAlt className="text-gray-400 dark:text-gray-500 group-hover:text-sky-500 dark:group-hover:text-sky-400 transition-colors text-lg opacity-75 group-hover:opacity-100 ml-auto self-start" />
      </div>
    </a>
  );
};
